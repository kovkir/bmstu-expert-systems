...
def buildGraph(
    ...
    prohibited_node_lst: list[Node] | None = None,
    ...
    for rule in rules:
        ...
        elif prohibited_rule_lst and rule in prohibited_rule_lst:
            dot.node(
                name=str(rule.number),
                shape="rectangle",
                color="orangered4", 
    ...
    for node in list(dict.fromkeys(nodes)):
        ...
        elif prohibited_node_lst and node in \
            [str(prohibited_node.number) for prohibited_node in prohibited_node_lst]:
            dot.node(
                name=node, 
                color="orangered4", 
                style="filled", 
